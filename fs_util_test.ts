...
Deno.test("[readTsFilePaths] all green", async () => {
  ...
  assertEquals(result[0], ["/foo/:name", expectRoot + "/foo/[name].ts"]);
  assertEquals(result[1], ["/foo/data_1", expectRoot + "/foo/data_1.ts"]);
  assertEquals(result[2], ["/foo/data_2", expectRoot + "/foo/data_2.ts"]);
});
